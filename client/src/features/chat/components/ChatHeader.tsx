...
import { PencilRuler, Video } from "lucide-react";
...
const ChatHeader = ({ onUserSelect }: ChatHeaderProps) => {
  ...
  const { data: conversationData, isLoading } = useGetChatHistory(
  ...
  const openWhiteboard = () => {
    ...
    const whiteboardUrl = `/whiteboard/${selectedConversationId}`;
    const windowFeatures =
      "width=1400,height=900,scrollbars=yes,resizable=yes,menubar=no,toolbar=no,location=no,status=no";

    window.open(whiteboardUrl, "_blank", windowFeatures);
  };
  ...
  return (
    <header className="flex items-center justify-between p-4 border-b border-border/40">
      ...
      >
        ...
        <div>
          <h2 className="text-lg font-bold text-customPrimary">
            {otherUser.fullName}
          </h2>
